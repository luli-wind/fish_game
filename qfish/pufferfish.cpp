...
pufferfish::pufferfish(const QString & fileName,QGraphicsScene *scene,int isLeft,int hp,int value):QFish(fileName,scene,isLeft,hp,value)
{
    // 初始化时加载所有帧
    for (int i = 1; i <= 4; ++i) {
        m_leftFrames << QPixmap(QString(":/image/pufferfish%1.png").arg(i));
        m_rightFrames << QPixmap(QString(":/image/pufferfish%1right.png").arg(i));
    }
    this->currentFrame=1;
}
void pufferfish::advance(int phase)
{
    if (!phase) return;
