...
lanternfish::lanternfish(const QString & fileName,QGraphicsScene *scene,int isLeft,int hp,int value):QFish(fileName,scene,isLeft,hp,value)
{
    // 初始化时加载所有帧
    for (int i = 1; i <= 8; ++i) {
        m_leftFrames << QPixmap(QString(":/image/lanternfish%1.png").arg(i));
        m_rightFrames << QPixmap(QString(":/image/lanternfish%1right.png").arg(i));
    }
    this->currentFrame=1;
}

void lanternfish::advance(int phase)
{
    if (!phase) return;
    ...
    int xspeed =  QRandomGenerator::global()->bounded(5)+2;
    int yspeed =QRandomGenerator::global()->bounded(8);
